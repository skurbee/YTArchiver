"""Shared yt-dlp options and environment; independent of orchestration."""
...
def find_yt_dlp() -> str | None:
    ...
    if p:
        return p
    # Check both the bundle/project root and its backend directory.
    candidates = [
        Path.cwd() / "yt-dlp.exe",
        Path(__file__).resolve().parent.parent / "yt-dlp.exe",
        Path(__file__).resolve().parent / "yt-dlp.exe",
