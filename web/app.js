...
(function () {
  ...
  // ─── Custom dropdown (yt-dd) — replaces <select> in settings ──────
  // Chromium/pywebview silently drops the selected-value text on native
  // <select> elements once any non-trivial CSS styling is applied, so
  // we build our own dropdown out of divs+spans. The original <select>
  ...
  function enhanceSelect(sel) {
    ...
    function onOutside(e) {
      if (!dd.contains(e.target)) closeMenu();
        ...
        // Re-register if click was inside but not on an option
        setTimeout(() => {
          document.addEventListener("click", onOutside, { once: true });
        }, 0);
      }
    ...
    // Also expose a hook so external code can force a repaint.
    sel._ytddRepaint = paintTrigger;

    paintTrigger();
    // Insert the widget immediately before the hidden select so it
    // takes the select's grid position.
  ...
  // Run on DOM ready and also after pywebviewready in case the settings
  // panel's selects are populated by Python just before we want to
  ...
  (function wireRedownloadSampleModal() {
    ...
    window.addEventListener("yt-control", (ev) => {
      ...
      m.hidden = false;
      startTick();
      // bug L-15: pause the auto-continue countdown while the user is
      // actively interacting with the modal (mouse hovering). Resumes
      // when they move the pointer away. Protects a user who is
  ...
  window.askChoice = function (opts) {
    return new Promise((resolve) => {
      ...
      const buttonsHtml = choices.map((c, i) => {
        // Support both `kind: "primary"|"danger"|"ghost"` AND
        // `primary: bool` / `danger: bool`. Default is now primary
        // (green) instead of ghost (gray) — per design feedback:
        // action choices should read as "go" buttons, with only
        // explicit `kind: "ghost"` dialed back to secondary styling.
  ...
  function showView(viewName) {
    ...
    // bug M-8: leaving the video grid — wipe the metadata-missing
  ...
  function _openSearchResultInWatch(state, seg) {
    ...
    api.browse_resolve_segment(state._jsonlPath || "",
      ...
      // audit D-34: surface resolve failures so the user sees WHY a
  ...
  async function drawGraph() {
    ...
    // bug M-13: backend reports `backfill_pending` when week-bucket
  ...
  function drawGraphFromData(data) {
    ...
    try {
      ...
      if (wrap && !wrap._graphResizeObs) {
        ...
        // audit D-33: gate the listener add with a flag on `wrap` the
  ...
  // Settings > Metadata tab — per-channel refresh status table.
  // Populates a sortable table showing, for every saved channel,
  // when views/likes and comments were last refreshed. Defaults to
  // oldest-first sort so stale channels float to the top. Per-row
  // actions queue a refresh; bulk row does everything at once.
  // Added 2026-04-23 alongside the bulk views-refresh rollout.
  ...
  function initEditChannelPanel() {
    ...
    const openPanel = (mode, channel) => {
      ...
      if (mode === "edit" && channel) {
        ...
        // bug H-1 (extended): compress sub-fields (level / output res /
        // batch size) were never loaded from the channel dict when the
        // panel opened, so the dropdowns showed HTML defaults. Any save
        // then stomped the real saved values with those defaults. Load
        // them alongside the checkbox.
    ...
    const collectPayload = () => {
      ...
      return {
        ...
        // bug H-1: the batch-size dropdown existed in the edit panel
        // but its value was never read here, so the user's choice was
        // silently ignored — every save wrote the old value back.
        // audit E-56: use Number.isFinite so "0" (user intending to
        // disable batching) isn't coerced back to 20 by the `|| 20`
        // fallback. 0 is a legitimate user value meaning "no batching".
  ...
  // audit D-22: previous body was:
  //   window._showToast?.(msg, "error") ?? alert(msg);
  // which fires alert() every time because _showToast returns
  // undefined on success and `undefined ?? x` evaluates to x. Every
  // validation error blocked the UI with a browser alert() modal on
  // top of the toast. The explicit if/else restores toast-only
  // behavior while still falling back to alert when the toast helper
  // isn't available.
  ...
  function initRecentContextMenu() {
    ...
    // audit D-30: keyboard handler for Recent table (Delete / Enter /
    ...
    tbody.addEventListener("contextmenu", (e) => {
      ...
      // audit F-37: trim textContent so indicator characters (●, ⚠)
      ...
      const items = n > 1 ? [
        { label: `Delete ${n} files`, cls: "dim",
          action: async () => {
            const ok = await askDanger(
              ...
              "Delete");
            if (!ok) return;
            // bug H-11: await each call instead of fire-and-forget, then
        ...
        { label: "Delete File", cls: "dim",
          action: async () => {
            ...
            if (!ok) return;
            // bug H-11: refresh Recent after delete so the row
      ...
      videoGrid.addEventListener("contextmenu", (e) => {
        ...
        showContextMenu(e.clientX, e.clientY, [
          ...
          { label: "Redownload\u2026", action: async () => {
            // audit D-24: prompt for resolution first (mirrors the
  ...
  // bug W-12: `initParentFolderPicker` was removed as dead legacy.
  // The Parent Folder input moved to Settings → General → Archive root
  // (ids settings-output-dir + settings-browse-output) and has its own
  // fully-wired handler. The former id `btn-browse-parent` / `parent-
  // folder-input` pair no longer exists in the HTML. Left as a noop
  // shim so the init bootstrap call doesn't throw if an old build
  // calls it.
  ...
  function initSubsContextMenu() {
    ...
    tbody.addEventListener("keydown", async (e) => {
      ...
      if (e.key === "Enter" || e.key === "F2") {
        ...
        // audit D-31: refresh in place instead of reloading the entire
  ...
  function onCtxKey(e) {
    // audit E-55: stopPropagation so the same Escape press doesn't
    // bubble up and close popovers / dialogs that happen to be
    // open underneath the context menu. Old behavior cascaded:
    // right-click inside a popover → Esc → ctx menu closed AND
    // popover closed, even though the user only meant to dismiss
    // the ctx menu.
    if (e.key === "Escape") {
      e.stopPropagation();
      closeContextMenu();
  ...
  function initUrlField() {
    ...
    if (!input || !btn) return;
    // bug W-2: populate the url-history datalist so the <input
    ...
    // bug W-8: persist the Download-tab preferences so they survive
    ...
    document.getElementById("vo-save-to-browse")?.addEventListener("click", async () => {
      ...
      if (res?.ok && res.path) {
        // bug W-6: if the user picks a path outside the archive tree,
  ...
  function initDragDropUrl() {
    ...
    if (!panel) return;
    // audit D-25: drag-counter pattern so hover state doesn't flicker
    ...
    panel.addEventListener("drop", async (e) => {
      ...
      if (window.pywebview?.api?.archive_single_video) {
        // audit D-26: pass the same readVideoOptions() dict the URL-
  ...
  function initKeyboardShortcuts() {
    document.addEventListener("keydown", (e) => {
      ...
      // Ctrl+F: focus Browse > Search input.
      // audit E-53: if Search view isn't currently visible, SWITCH to
      // it + focus. Old behavior silently no-opped when Search view
      // was hidden (no preventDefault, no action), so the user's
      // Ctrl+F press did nothing most of the time.
      if ((e.ctrlKey || e.metaKey) && e.key === "f") {
        ...
        if (viewSearch && viewSearch.style.display === "none") {
          // Switch to Browse tab, then into Search submode if the
          // submode toggle exists. Focus after a tick so the layout
          // settles and the input is ready to receive focus.
          browseTab?.click();
      ...
      // audit E-54: actually focus the filter input after clicking
      // the tab. Old behavior only switched tabs — the comment said
      // "jump to Subs + focus filter" but the focus step was missing.
  ...
  function initWatchActions() {
    ...
    const vEl = document.getElementById("watch-video");
    // bug M-5: speed used to reset to 1.0x on every video switch because
    // it wasn't persisted. Mirror the volume-persistence pattern
    ...
    try {
      // audit F-10: use Number.isFinite so a stored "0" (or any
      // other falsy-but-valid number) isn't coerced back to 1.
    ...
    vEl?.addEventListener("loadedmetadata", () => {
      try {
        // audit F-10: same Number.isFinite guard as above.
    ...
    document.addEventListener("keydown", (e) => {
      ...
      // audit D-29: include TEXTAREA so typing in a bookmark-note
      // textarea doesn't trigger Space/Arrow/B/M video shortcuts
      // (typing a space paused the video; arrow keys jumped
      // playback instead of moving the cursor).
    ...
    document.getElementById("btn-watch-redownload")?.addEventListener("click", async () => {
      ...
      if (!pick) return;
      // bug L-10: whitelist the resolution string before forwarding to
      // the backend. The dialog only presents valid options, but a
      // future refactor could let other values through — this guards
      // against a `pick` that the backend wouldn't understand.
  ...
  function initSettingsTab() {
    ...
    // quality tier for the chosen output resolution.
    (function wireCompressDryRun() {
      ...
      const _render = (data) => {
        ...
        html += `</tr>`;
        // audit D-28: null-safe numeric reads. A single malformed
    ...
    // feature H-2: transcript drift scan + fix modal. Opens with a
    // channel dropdown populated from get_subs_channels; Scan returns
    // the three drift categories; Fix applies all three (reconstruct
    // .txt from .jsonl, queue Whisper for .txt-only entries, rebuild
    // FTS if phantoms). Results render inline; Fix button stays
    // disabled until a scan with nonzero drift has returned.
    (function wireDriftScan() {
    ...
    // Repair YT auto-captions modal (v64.7 parser fix). Channel
    // dropdown (or "All channels"), dry-run + include-punctuated
    // toggles. On Run, kicks off the background worker via
    // api.repair_yt_captions and closes the modal — progress
    // streams to the main log.
    (function wireRepairYtCaptions() {
    ...
    // Restore transcript punctuation modal — same shape as the Repair
    // YT captions modal. Channel dropdown (or "All channels"), dry-run
    // toggle. On Run, calls api.punct_restore_segments and the task
    // queues on the sync queue for pause/resume/cancel.
    (function wirePunctRestore() {
  ...
  function initDiagnosticsDialog() {
    ...
    async function run() {
      // audit F-39: early bail if rowsEl is missing (DOM out of sync
  ...
  async function _reconcileMissingFolders() {
    ...
    for (const ch of missing) {
      // audit D-21: askChoice takes a SINGLE opts object, not three
  ...
  function boot() {
    ...
    try { initMetadataTab(); } catch(e){console.error("initMetadataTab:",e);}
