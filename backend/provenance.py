...
def _title_id_map(jsonl_path: str) -> dict[str, set[str]]:
    ...
    try:
        ...
        _log.warning("Could not read transcript sidecar %s: %s", jsonl_path, exc)
    return out
...
def _upgrade_txt_file(txt_path: str, dry_run: bool) -> dict:
    ...
    with txt_lock_for(txt_path):
        ...
        for i, line in enumerate(lines):
            ...
            eol = line[len(ls) :]
            rebuilt = (
                f"===({m.group(1)}), {m.group(2)}, {m.group(3)}, {m.group(4)}, (youtu.be/{vid})==="
            )
        ...
        if stats["changed"] and not dry_run:
            hide = None
            if was_hidden:
                # Replacement can strip the attribute. Hide the stage before
                # install and the final path afterward so there is no visible
                # window for per-video transcript sidecars.
                from .utils import hide_file_win as hide
            atomic_write_text(
                txt_path,
                "".join(lines),
                before_replace=hide,
...
def _channel_dirs(output_dir: str, channel_folder: str | None) -> list[str]:
    ...
    try:
        for e in os.scandir(output_dir):
            if e.is_dir() and not e.name.startswith(".") and e.name != "YTArchiver Info":
                out.append(e.path)
    except OSError as e:
...
def _mp4_worklist(
    output_dir: str, channel_folder: str | None, db_path=None
    ...
    scope_root = os.path.join(output_dir, channel_folder) if channel_folder else output_dir
...
def _embed_one(ffmpeg: str, path: str, video_id: str, title: str, channel: str) -> tuple[bool, str]:
    ...
    cmd = [
        ...
        "-metadata",
        f"artist={channel}" if channel else "artist=",
        "-metadata",
        f"date={date_iso}",
        "-metadata",
        f"comment={url}",
    ...
    try:
        ...
        if not (lo <= new_size <= hi):
            ...
            pkts_ok = (
                src_pkts is not None
                and out_pkts is not None
            ...
            if not (pkts_ok and new_size < lo):
                ...
                if ffprobe is None:
                    return False, (
                        f"size sanity failed, ffprobe unavailable "
                        f"to verify ({st.st_size} -> {new_size} "
                        f"bytes)"
                    )
                if src_pkts is None or out_pkts is None:
                    return False, (
                        f"size sanity failed, packet count "
                        f"unverifiable ({st.st_size} -> "
                        f"{new_size} bytes)"
                    )
                return False, (
                    f"size+content sanity failed ({st.st_size} -> "
                    f"{new_size} bytes, {src_pkts} -> {out_pkts} "
                    f"video packets)"
                )
            _log.info(
                "provenance: %s shrank %d -> %d bytes but all %d video "
...
def embed_provenance_archive(
    ...
    def _clear_progress():
        ...
        try:
            ...
            stream.emit(
                [
                    [
                        _json.dumps({"kind": "clear_line", "marker": "provenance_progress"}),
                        "__control__",
                    ]
                ]
            )
    ...
    if do_txt:
        ...
        for ci, ch_dir in enumerate(ch_dirs):
            ...
            if queues is not None:
                try:
                    queues.set_sync_pass_progress(ci, len(ch_dirs))
                except Exception as e:
                    swallow("provenance progress", e)
            _progress(
                f"    [{ci + 1}/{len(ch_dirs)}] "
            ...
            for dirpath, dns, fns in os.walk(ch_dir):
                ...
                for fn in fns:
                    ...
                    if _cancelled():
                        return res
                    try:
                        st = _upgrade_txt_file(os.path.join(dirpath, fn), dry_run)
                        ...
                        res["failed"] += 1
                        _emit(
                            [["  — ", ["dim"]], [f"header upgrade failed for {fn}: {e}\n", ["red"]]]
                        )
        _clear_progress()
        _emit(
            [
    ...
    if do_mp4:
        ...
        if not dry_run:
            try:
                ...
                swallow("provenance ledger directory", e)
        ...
        for wi, (fp, vid, title, channel) in enumerate(work):
            ...
            if queues is not None:
                try:
                    queues.set_sync_pass_progress(wi, len(work))
                except Exception as e:
                    swallow("provenance progress", e)
            try:
                st = os.stat(fp)
            ...
            if ok:
                ...
                if res["failed"] <= 20:
                    _emit([["  — ", ["dim"]], [f"{os.path.basename(fp)}: {err}\n", ["red"]]])
            ...
            now = time.time()
            if (wi + 1) % 10 == 0 or (now - t_last_emit) >= 2 or wi + 1 == len(work):
                t_last_emit = now
                _progress(
                    f"    [{wi + 1:,}/{len(work):,}] "
                    f"Embedding tags — "
    ...
    if queues is not None:
        try:
            queues.set_sync_pass_progress(0, 0)
        except Exception as e:
            swallow("provenance progress reset", e)
    return res
