...
class AutoBackupRetentionTests(unittest.TestCase):
    def test_rotation_is_bounded_and_only_prunes_scheduled_backups(self) -> None:
        from backend import auto_backup

        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            info_dir = root / auto_backup.INFO_DIR_NAME
            info_dir.mkdir()

            generated = []
            for day in range(1, 8):
                path = info_dir / (
                    f"{auto_backup.BACKUP_PREFIX}2026-08-{day:02d}_120000.zip")
                path.write_bytes(str(day).encode("ascii"))
                generated.append(path)

            # Prefix lookalikes and user-created ZIPs are outside retention.
            lookalike = info_dir / "ytarchiver_backup_keep_this.zip"
            manual = info_dir / "my_manual_backup.zip"
            lookalike.write_bytes(b"user")
            manual.write_bytes(b"user")

            # Pre-fix builds moved expired scheduled ZIPs here. Rotation must
            # clean those up or total disk use would still grow forever.
            legacy_trash = (
                root / ".YTArchiver Trash" / auto_backup.INFO_DIR_NAME)
            legacy_trash.mkdir(parents=True)
            legacy_plain = (
                legacy_trash /
                "ytarchiver_backup_2026-07-01_120000.zip")
            legacy_collision = (
                legacy_trash /
                "ytarchiver_backup_2026-07-01_120000.1234567890.zip")
            unrelated_trash = legacy_trash / "notes.zip"
            for path in (legacy_plain, legacy_collision, unrelated_trash):
                path.write_bytes(b"trash")

            removed = auto_backup._rotate_backups(str(info_dir), str(root))

            self.assertEqual(removed, 5)
            self.assertEqual(
                [path.name for path in generated if path.exists()],
                [path.name for path in generated[-auto_backup.KEEP_BACKUPS:]],
            )
            self.assertTrue(lookalike.exists())
            self.assertTrue(manual.exists())
            self.assertFalse(legacy_plain.exists())
            self.assertFalse(legacy_collision.exists())
            self.assertTrue(unrelated_trash.exists())
...
class ArchiveMixinTests(unittest.TestCase):
    ...
    def test_dltrack_duration_seconds_requires_positive_number(self) -> None:
        self.assertEqual(
            archive_mixin.dltrack_duration_seconds({"duration": "600.5"}),
            600.5)
        self.assertIsNone(
            archive_mixin.dltrack_duration_seconds({"duration": "NA"}))
        self.assertIsNone(
            archive_mixin.dltrack_duration_seconds({"duration": "0"}))
    ...
    def test_check_disk_space_fails_closed_on_probe_error(self) -> None:
        ...
        with mock.patch.object(fs_safety.shutil, "disk_usage",
            ...
            with self.assertLogs(fs_safety._log, level="WARNING"):
                self.assertFalse(utils.check_disk_space("X:/Archive", 1))
...
class YouTubeSessionGuardTests(unittest.TestCase):
    ...
    def test_passive_log_scanner_ignores_rate_limit_words_in_title(self) -> None:
        stream = mock.Mock()
        pause_event = threading.Event()
        queue_state = mock.Mock()
        youtube_session.configure(stream, pause_event, queue_state)
        ...
        youtube_session.scan_log_line(
            "      — ✓ Too Many Requests (54.2 MB)")

        self.assertFalse(youtube_session.rate_limit_detected())
    ...
    def test_scoped_views_refresh_clears_progress_when_nothing_is_recent(
        ...
        with tempfile.TemporaryDirectory() as td, \
                ...
                mock.patch.object(refresh_views, "_filter_recent_on_disk",
                                  return_value=[]), \
                mock.patch.object(refresh_views, "stamp_channel_refresh") \
                as stamp_refresh, \
        ...
        stamp_refresh.assert_called_once_with(
            channel, "last_views_refresh_ts")

    def test_scoped_comments_refresh_stamps_when_no_videos_match(
            self) -> None:
        stream = mock.Mock()
        channel = {
            "name": "Old Channel",
        ...
        with tempfile.TemporaryDirectory() as td, \
                mock.patch.object(refresh_comments, "_folder_for_channel",
                                  return_value=Path(td)), \
                mock.patch.object(refresh_comments, "find_yt_dlp",
                                  return_value="yt-dlp"), \
                mock.patch.object(refresh_comments, "_scan_channel_videos",
                                  return_value=[]), \
                mock.patch.object(refresh_comments, "stamp_channel_refresh") \
                as stamp_refresh:
            result = refresh_comments.refresh_channel_comments(
                channel, stream, only_recent_days=30)

        self.assertTrue(result["ok"])
        self.assertEqual(result["fetched"], 0)
        stamp_refresh.assert_called_once_with(
            channel, "last_comments_refresh_ts")

    def test_channel_refresh_stamp_updates_matching_saved_channel(self) -> None:
        channel = {
            "name": "Channel",
            "url": "https://example.test/channel/",
        }
        saved = {"channels": [{
            "name": "Channel",
            "url": "https://example.test/channel",
            "last_views_refresh_ts": 1,
        }]}
        with mock.patch.object(
                ytarchiver_config, "config_transaction",
                return_value=contextlib.nullcontext(saved)), \
                mock.patch.object(refresh_state.time, "time",
                                  return_value=1234.5):
            stamped = refresh_state.stamp_channel_refresh(
                channel, "last_views_refresh_ts")

        self.assertTrue(stamped)
        self.assertEqual(
            saved["channels"][0]["last_views_refresh_ts"], 1234.5)
...
class YtDlpEventsTests(unittest.TestCase):
    ...
    def test_extract_video_id_from_line_accepts_all_alpha_id(self) -> None:
        self.assertEqual(
            extract_video_id_from_line("[youtube] abcdefghijk: Downloading"),
            "abcdefghijk",
        )
...
class TextUtilsTests(unittest.TestCase):
    ...
    def test_extract_video_id_accepts_all_alpha_db_fallback(self) -> None:
        conn = sqlite3.connect(":memory:")
        try:
            path = r"X:\Archive\Channel\Video.mp4"
...
class ChannelMixinTests(unittest.TestCase):
    ...
    def test_chan_fix_dates_cancel_sets_active_event(self) -> None:
        ...
        # so nothing could ever stop a running pass. It now lives on the
        # Api instance and chan_fix_dates_cancel sets it.
        mixin = ChannelMixin()
        mixin._queues = mock.Mock(current_sync=None, current_gpu=None)
        mixin._log_stream = mock.Mock()
        started = threading.Event()
        captured = {}

        def fake_fix_dates(folder, stream, cancel_event=None):
        ...
        with tempfile.TemporaryDirectory() as td, \
                ...
                mock.patch.object(channel_mixin, "load_config",
                                  return_value={"output_dir": td}), \
                mock.patch.object(channel_mixin.reorg_backend,
                                  "fix_file_dates",
                                  side_effect=fake_fix_dates):
            result = mixin.chan_fix_file_dates({"name": "Chan"})
            self.assertTrue(started.wait(timeout=1.0))
            # A second start while one is running must be refused
            # (single-slot guard).
            second = mixin.chan_fix_file_dates({"name": "Chan"})
            cancel_result = mixin.chan_fix_dates_cancel()

        self.assertTrue(result["ok"])
        self.assertFalse(second["ok"])
        self.assertIn("already running", second["error"])
        self.assertTrue(cancel_result["ok"])
        self.assertTrue(cancel_result["running"])
...
class ThumbnailMixinTests(unittest.TestCase):
    ...
    def test_sweep_invalidates_thumb_cache_when_fetched(self) -> None:
        ...
        self.assertIn("invalidate_channel_videos",
                      _ops.sweep_missing_thumbnails.__code__.co_names)
...
class SubsTests(unittest.TestCase):
    ...
    def test_subs_queue_pending_worker_uses_fresh_config(self) -> None:
        ...
        cfg0 = {"output_dir": "X:/Archive", "channels": []}
        fresh = {
...
class BrowseMixinTests(unittest.TestCase):
    ...
    def test_channel_grid_reports_unfinished_redownload(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            channel_dir = Path(td) / "Channel One"
            channel_dir.mkdir()
            (channel_dir / "_redownload_progress.json").write_text(
                json.dumps({"resolution": "480", "done_ids": ["abc"]}),
                encoding="utf-8")
                ...
                def _browse_fresh_config(self):
                    return {
                        ...
                        "channels": [{
                            "name": "Channel One",
                            "folder": "Channel One",
                            "url": "https://youtube.com/@channelone",
                        }],
                    }

            stats = {"n_vids": 1, "size_bytes": 10, "size_gb": 0.0}
            with mock.patch.object(
                    browse_mixin.archive_scan, "load_disk_cache",
                    return_value={}), \
                    mock.patch.object(
                        browse_mixin.archive_scan, "stats_for_channel",
                        return_value=stats), \
                    mock.patch.object(
                        browse_mixin.index_backend, "_reader_open",
                        return_value=None), \
                    mock.patch.object(channel_art, "ensure_banner_thumb",
                                      return_value=None), \
                    mock.patch.object(channel_art, "banner_path_for",
                                      return_value=None), \
                    mock.patch.object(channel_art, "ensure_avatar_thumb",
                                      return_value=None), \
                    mock.patch.object(channel_art, "avatar_path_for",
                                      return_value=None):
                rows = Api()._browse_list_channels_impl()

        self.assertEqual(len(rows), 1)
        self.assertTrue(rows[0]["_pending_redownload"])
        self.assertEqual(rows[0]["_redownload_res"], "480")
    ...
    def test_manual_videos_defer_local_duration_for_folder_fallback(
            self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            with mock.patch("backend.index.list_manual_videos",
                    ...
                    mock.patch("backend.api_mixins.browse_mixin.subprocess.run",
                               side_effect=AssertionError(
                                   "initial list must not run ffprobe")), \
                    mock.patch.object(
                        Api, "_queue_manual_duration_backfill") as queue_dur, \
                    mock.patch.object(
                        Api, "_queue_manual_local_thumbnail_backfill"):
                result = Api().list_manual_videos(sort="title")

        self.assertFalse(result["rows"][0].get("duration"))
        queue_dur.assert_called_once()
        self.assertEqual(queue_dur.call_args.args[0][0]["filepath"], str(video))
        self.assertTrue(
            queue_dur.call_args.args[0][0]["register_if_missing"])

    def test_manual_duration_backfill_catalogs_folder_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            video = Path(tmp) / "Loose Manual.mp4"
            video.write_bytes(b"video")
            ...
            def mark_registered(*_args, **_kwargs):
                registered.set()
                return True

            with mock.patch.object(
                    browse_mixin, "_probe_manual_duration_seconds",
                    return_value=65.5), \
                    mock.patch("backend.index.set_video_duration",
                               return_value=False) as set_duration, \
                    mock.patch("backend.index.register_video",
                               side_effect=mark_registered) as register:
                ...
                self.assertTrue(registered.wait(5),
                                "duration backfill did not finish")

        set_duration.assert_called_once_with(str(video), 65.5)
        register.assert_called_once_with(
            str(video), "Single Videos", "Loose Manual",
            tx_status="pending", video_id=None, duration_secs=65.5)

    def test_manual_thumbnail_backfill_finds_ready_sidecars_before_generation(
            self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            slow = root / "Needs Generation.mp4"
            ready = root / "Already Ready.mp4"
            generated = root / "Needs Generation.local.jpg"
            sidecar = root / "Already Ready.jpg"
            slow.write_bytes(b"video")
            ready.write_bytes(b"video")
            generated.write_bytes(b"jpg")
            sidecar.write_bytes(b"jpg")

            events = []

            def find_thumb(filepath, _video_id):
                events.append(("find", Path(filepath).name))
                return str(sidecar) if filepath == str(ready) else None

            def generate(filepath, *_args, **_kwargs):
                events.append(("generate", Path(filepath).name))
            ...
            with mock.patch("backend.index.find_thumbnail",
                            side_effect=find_thumb), \
                    mock.patch("backend.index._file_url",
                               side_effect=lambda p: f"file:///{p}"), \
                    mock.patch("backend.thumbnails._ensure_thumbnails_dir",
                               return_value=str(root)), \
                    mock.patch("backend.thumbnails._generate_local_thumbnail",
                               side_effect=generate), \
                    mock.patch.object(browse_mixin.threading, "Thread",
                                      side_effect=ImmediateThread):
                Api()._queue_manual_local_thumbnail_backfill([
                    {"filepath": str(slow), "title": slow.stem,
                     "video_id": ""},
                    {"filepath": str(ready), "title": ready.stem,
                     "video_id": "abc123def45"},
                ])

        self.assertEqual(events, [
            ("find", "Needs Generation.mp4"),
            ("find", "Already Ready.mp4"),
    ...
    def test_manual_videos_badges_transcript_from_local_sidecar(self) -> None:
        ...
        queue_thumb.assert_called_once()
        self.assertEqual(queue_thumb.call_args.args[0][0]["filepath"], str(video))
    ...
    def test_manual_videos_badges_no_speech_is_neutral(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            with mock.patch("backend.index.list_manual_videos",
                    ...
                    mock.patch("backend.index.list_manual_duplicate_filepaths",
                               return_value=[]), \
                    mock.patch(
                        "backend.api_mixins.browse_mixin._is_system_temp_path",
                        return_value=False):
                result = Api().list_manual_videos(sort="title")
    ...
    def test_list_all_videos_clamps_limit_and_coerces_offset(self) -> None:
        ...
        with mock.patch("backend.api_mixins.recent_mixin.index_backend"
                ...
                mock.patch("backend.api_mixins.recent_mixin.index_backend"
                           ".backfill_downloaded_ts_from_recent"):
...
class VideoMixinTests(unittest.TestCase):
    def test_video_delete_removes_stale_catalog_row_when_file_is_gone(self) -> None:
        mixin = VideoMixin()
...
class LogRowsTests(unittest.TestCase):
    ...
    def test_sync_row_index_has_no_leading_padding(self) -> None:
        ...
        log_rows._sync_row_emit(stream, 99, 111, "Channel")
        segments = stream.emit.call_args.args[0]
        rendered = "".join(segment[0] for segment in segments)

        self.assertTrue(rendered.startswith("[99/111] "))
...
class RecentTrackTests(unittest.TestCase):
    ...
    def test_record_recent_download_keeps_same_title_different_video_id(
        ...
        with mock.patch("backend.ytarchiver_config.config_is_writable",
                ...
                mock.patch("backend.ytarchiver_config.config_transaction",
                           return_value=self._tx(cfg)), \
                mock.patch("backend.index.record_video_download",
                           return_value=True) as record_catalog:
        ...
        self.assertEqual(cfg["recent_downloads"][0]["video_id"],
                         "BBBBBBBBBBB")
        record_catalog.assert_called_once()
        self.assertEqual(record_catalog.call_args.kwargs["video_id"],
                         "BBBBBBBBBBB")
...
class MediaOpsMixinTests(unittest.TestCase):
    ...
    def test_archive_rescan_reports_real_progress_and_completion(self) -> None:
        ...
        with mock.patch("backend.api_mixins.media_ops_mixin"
                ...
                mock.patch("backend.api_mixins.media_ops_mixin.os.path.isdir",
                           return_value=True), \
    ...
    def test_archive_rescan_cancels_when_archive_root_is_unreachable(self) -> None:
        class Api(MediaOpsMixin):
            def sync_is_running(self):
                return False
        ...
        api = Api()
        api._config = {"output_dir": "X:/Archive", "channels": []}
        api._log_stream = mock.Mock()
        api._window = None
        ...
        with mock.patch("backend.api_mixins.media_ops_mixin"
                ...
                mock.patch("backend.api_mixins.media_ops_mixin.os.path.isdir",
                           return_value=False), \
                mock.patch("backend.api_mixins.media_ops_mixin"
                           ".index_backend.prune_missing_videos") as prune, \
                mock.patch("backend.api_mixins.media_ops_mixin.threading.Thread",
                           ImmediateThread):
            result = api.archive_rescan()
        ...
        api._log_stream.emit_error.assert_called_once()
        self.assertIn("not reachable", api._log_stream.emit_error.call_args.args[0])
        state = api.archive_rescan_state()
        self.assertFalse(state["running"])
...
class SyncCoreTests(unittest.TestCase):
    ...
    def test_channel_folder_has_media_probe(self) -> None:
        # [H1] helper: empty/missing folder → False (must not quick-skip);
        # populated folder → True; unresolvable config → fail-open True.
        sync_all_module = __import__(
            "backend.sync.sync_all", fromlist=["sync_all"])
        probe = sync_all_module._channel_folder_has_media
        with tempfile.TemporaryDirectory() as td:
            cfg = {"output_dir": td}
            ch = {"name": "Chan", "url": "https://www.youtube.com/@chan"}
            # Folder missing entirely.
            self.assertFalse(probe(cfg, ch))
            folder = Path(td) / "Chan"
            folder.mkdir()
            # Folder exists but is empty.
            self.assertFalse(probe(cfg, ch))
            # Sidecars / partial artifacts alone don't count as media.
            (folder / "Chan Transcript.txt").write_text("x", encoding="utf-8")
            (folder / "clip.mp4.part").write_bytes(b"x")
            self.assertFalse(probe(cfg, ch))
            # A real media file (year-split layout) flips it True.
            year_dir = folder / "2024"
            year_dir.mkdir()
            (year_dir / "video.mp4").write_bytes(b"x")
            self.assertTrue(probe(cfg, ch))
    ...
    def _run_sync_all_quickcheck_case(
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            archive = Path(td) / "archive.txt"
            archive.write_text("youtube abcdefgh123\n", encoding="utf-8")
            try:
                with mock.patch.object(q, "save_debounced",
                                       return_value=None), \
                        mock.patch.object(
                            sync_all_module, "load_config",
                            return_value={"channels": [channel],
                                          "output_dir": td}), \
                        mock.patch.object(sync_all_module, "ARCHIVE_FILE",
                                          str(archive)), \
    ...
    def test_quickcheck_skips_populated_channel(self) -> None:
        # Baseline: populated folder + all-archived probe → quick check
        # runs and sync_channel is skipped (fast path intact).
        calls = self._run_sync_all_quickcheck_case(make_media=True)
        self.assertEqual(calls["qc"], 1)
        self.assertEqual(calls["sync"], 0)

    def test_quickcheck_never_skips_emptied_channel(self) -> None:
        # [H1]: an EMPTIED channel folder must fall through to
        # sync_channel (whose repopulate branch bypasses the download
        # archive) instead of being quick-skipped as "no new videos".
...
class QuickcheckTests(unittest.TestCase):
    ...
    def test_quick_check_treats_duration_filtered_ids_as_known(self) -> None:
        ...
        self.assertEqual(result["fresh_ids"], [])
        self.assertEqual(result["filtered_ids"], ["SHORTVIDEO1"])
...
class ConfigTests(unittest.TestCase):
    def test_load_config_returns_defaults_when_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertFalse(cfg["legacy_subs_tab"])

    def test_existing_config_without_dense_setting_migrates_enabled(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg_file = Path(td) / "ytarchiver_config.json"
            cfg_file.write_text(json.dumps({
            ...
            self.assertTrue(loaded["legacy_subs_tab"])
            self.assertTrue(persisted["legacy_subs_tab"])

    def test_existing_dense_setting_is_never_overridden(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg_file = Path(td) / "ytarchiver_config.json"
            cfg_file.write_text(json.dumps({
                "_migration_v2_pending_tx_ids": True,
        ...
        recent = [
            {
                "title": f"Video {i}",
                "channel": "Channel",
                "filepath": f"X:/Archive/video-{i}.mp4",
                "download_ts": i,
            }
            for i in range(1000)
        ]
    ...
    def test_legacy_subs_tab_setting_round_trip(self) -> None:
        ...
        fresh = {"log_mode": "Simple", "legacy_subs_tab": False}
        with mock.patch.object(settings_mixin, "config_is_writable",
                               return_value=True), \
                mock.patch.object(settings_mixin, "load_config",
        ...
        self.assertFalse(loaded["legacy_subs_tab"])
        self.assertTrue(result["ok"])
        self.assertTrue(saved[-1]["legacy_subs_tab"])
...
class FrontendShortcutSourceTests(unittest.TestCase):
    def test_search_shortcuts_use_real_submode_selector(self) -> None:
        root = Path(__file__).resolve().parents[1]
        shortcuts = (root / "web" / "shortcuts.js").read_text(
            encoding="utf-8")
        palette = (root / "web" / "commandPalette.js").read_text(
            encoding="utf-8")
    ...
    def test_ctrl_letter_shortcuts_normalize_capslock(self) -> None:
        root = Path(__file__).resolve().parents[1]
        shortcuts = (root / "web" / "shortcuts.js").read_text(
            encoding="utf-8")

        self.assertIn("e.key.toLowerCase()", shortcuts)
        for letter in ("q", "f", "l", "s", "k", "p"):
...
class QueueStateTests(unittest.TestCase):
    def test_queue_payload_omits_legacy_write_only_order(self) -> None:
        q = queues.QueueState()
        try:
            q.sync = [{"name": "Channel", "url": "https://example.test/c"}]
            q.gpu = [{"path": "video.mp4"}]

            with q._lock:
                payload = q._build_save_payload_locked()
    ...
    def test_sync_payload_uses_running_status_label(self) -> None:
        ...
        try:
            ...
            self.assertEqual(
                row["name"], "Metadata finalizing \u2014 Example Channel")
...
class DiskWatchTests(unittest.TestCase):
    ...
    def test_retry_tick_recovery_resumes_once_when_raced(self) -> None:
        ...
        on_resume = mock.Mock()
        monitor = disk_watch.DiskErrorMonitor(
            stream, mock.Mock(), on_resume, lambda: "X:\\Archive")
        monitor._active = True
        monitor._path = "X:\\Archive"
...
class IndexIngestTests(unittest.TestCase):
    ...
    def test_register_video_rejects_partial_missing_and_zero_byte(self) -> None:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
            db_path = Path(td) / "transcription_index.db"
            partial = Path(td) / "Interrupted.temp.mp4"
            partial.write_bytes(b"partial")
            empty = Path(td) / "Empty.mp4"
            empty.write_bytes(b"")
            missing = Path(td) / "Missing.mp4"

            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path):
                self._reset_index_module()
                self.assertFalse(index.register_video(
                    str(partial), "Channel", "Interrupted"))
                self.assertFalse(index.register_video(
                    str(empty), "Channel", "Empty"))
                self.assertFalse(index.register_video(
                    str(missing), "Channel", "Missing"))
                conn = index._open()
                self.assertEqual(conn.execute(
                    "SELECT COUNT(*) FROM videos").fetchone()[0], 0)
                self._reset_index_module()

    def test_recent_sort_uses_completed_download_not_discovery(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = Path(td) / "transcription_index.db"
            downloaded = Path(td) / "Actually Downloaded.mp4"
            rescanned = Path(td) / "Old File Found Today.mp4"
            downloaded.write_bytes(b"video")
            rescanned.write_bytes(b"video")

            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path):
                self._reset_index_module()
                self.assertTrue(index.register_video(
                    str(downloaded), "New Channel", "Actually Downloaded",
                    video_id="new12345678"))
                self.assertTrue(index.register_video(
                    str(rescanned), "Old Channel", "Old File Found Today",
                    video_id="old12345678"))
                conn = index._open()
                conn.execute(
                    "UPDATE videos SET added_ts=100 WHERE filepath=?",
                    (str(downloaded),))
                conn.execute(
                    "UPDATE videos SET added_ts=1000 WHERE filepath=?",
                    (str(rescanned),))
                conn.commit()

                self.assertTrue(index.record_video_download(
                    str(downloaded), video_id="new12345678",
                    downloaded_ts=500, duration_secs=125))
                rows = index.list_all_videos(
                    sort="recent", limit=10, include_thumbs=False)["rows"]

                self.assertEqual([r["title"] for r in rows],
                                 ["Actually Downloaded", "Old File Found Today"])
    ...
    def test_recent_config_backfills_catalog_download_timestamp(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = Path(td) / "transcription_index.db"
            video = Path(td) / "Legacy Recent.mp4"
            video.write_bytes(b"video")
            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path):
                self._reset_index_module()
                self.assertTrue(index.register_video(
                    str(video), "Channel", "Legacy Recent",
    ...
    def test_partial_catalog_rows_are_hidden_from_browse(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = Path(td) / "transcription_index.db"
            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path):
                self._reset_index_module()
                conn = index._open()
                conn.execute(
                    "INSERT INTO videos(filepath,title,channel,availability) "
                    "VALUES (?,?,?,'partial')",
                    (str(Path(td) / "Ghost.temp.mp4"), "Ghost.temp", "Channel"))
                conn.commit()
                rows = index.list_all_videos(
                    sort="newest", limit=10, include_thumbs=False)["rows"]
                self.assertEqual(rows, [])
                self._reset_index_module()

    def test_schema_reopen_quarantines_legacy_temp_row(self) -> None:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
            db_path = Path(td) / "transcription_index.db"
            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path):
                self._reset_index_module()
                ...
                availability = conn.execute(
                    "SELECT availability FROM videos").fetchone()[0]
                self.assertEqual(availability, "partial")
                self.assertEqual(index.list_all_videos(
                    include_thumbs=False)["rows"], [])
                self._reset_index_module()

    def test_sweep_reconciles_seen_and_missing_catalog_state(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = Path(td) / "transcription_index.db"
            output = Path(td) / "archive"
            channel_dir = output / "Channel"
            channel_dir.mkdir(parents=True)
            present = channel_dir / "Present.mp4"
            missing = channel_dir / "Missing.mp4"
            ...
            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path), \
                ...
                self._reset_index_module()
                self.assertTrue(index.register_video(
                    str(present), "Channel", "Present",
                    video_id="pre12345678"))
                self.assertTrue(index.register_video(
                    str(missing), "Channel", "Missing",
                    video_id="mis12345678"))
                conn = index._open()
                conn.execute(
                    "UPDATE videos SET availability='missing' WHERE filepath=?",
                    (str(present),))
                conn.commit()
                missing.unlink()

                result = index.sweep_new_videos(
                    str(output), [{"name": "Channel", "url": "channel-url"}])
                states = dict(conn.execute(
                    "SELECT title, availability FROM videos").fetchall())

                self.assertEqual(states["Present"], "available")
                self.assertEqual(states["Missing"], "missing")
                self.assertEqual(result["availability_restored"], 1)
                self.assertEqual(result["availability_missing"], 1)
                self.assertEqual([r["title"] for r in index.list_all_videos(
                    include_thumbs=False)["rows"]], ["Present"])
                self._reset_index_module()

    def test_incomplete_sweep_does_not_mark_rows_missing(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = Path(td) / "transcription_index.db"
            output = Path(td) / "archive"
            channel_dir = output / "Channel"
            blocked = channel_dir / "blocked"
            blocked.mkdir(parents=True)
            vanished = channel_dir / "Vanished.mp4"
            vanished.write_bytes(b"video")
            ...
            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path), \
                ...
                self._reset_index_module()
                self.assertTrue(index.register_video(
                    str(vanished), "Channel", "Vanished",
                    video_id="van12345678"))
                vanished.unlink()

                real_scandir = os.scandir
                def guarded_scandir(path):
                    if os.path.normcase(os.fspath(path)) == os.path.normcase(str(blocked)):
                        raise PermissionError("simulated archive read failure")
                    return real_scandir(path)

                with mock.patch("backend.index_maintenance.os.scandir",
                ...
                conn = index._open()
                availability = conn.execute(
                    "SELECT availability FROM videos WHERE title='Vanished'"
                ).fetchone()[0]
                self.assertEqual(availability, "available")
                self.assertEqual(result["availability_missing"], 0)
                self._reset_index_module()
    ...
    def test_list_videos_for_channel_uses_index_without_per_row_file_stats(
        ...
        with tempfile.TemporaryDirectory() as td:
            db_path = Path(td) / "transcription_index.db"
            missing_video = (
                r"X:\Archive\Large Channel\2026\06 June"
    ...
    def test_prune_missing_videos_aborts_suspicious_mass_prune(self) -> None:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
            ...
            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path):
                ...
                assert conn is not None
                rows = [
                    (str(Path(td) / f"missing-{i}.mp4"), f"Missing {i}",
                     "Channel", f"vid{i:08d}"[-11:], 123)
                    for i in range(60)
                ]
                conn.executemany(
                    "INSERT INTO videos("
                ...
                with mock.patch.object(index_maintenance.os.path, "isfile",
                                       return_value=False):
                    result = index_maintenance.prune_missing_videos()

                self.assertEqual(result["aborted_suspicious"], 60)
...
class NoSpeechStatusTests(unittest.TestCase):
    ...
    def test_no_speech_persist_lookup_counts_and_archive_pct(self) -> None:
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
            ...
            # Real YouTube ids are 11 chars; use valid ids embedded in the
            # filename as a belt-and-suspenders identity source.
    ...
    def test_title_only_status_lookup_is_scoped_to_channel(self) -> None:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
            db_path = Path(td) / "transcription_index.db"
            first = Path(td) / "first.mp4"
            second = Path(td) / "second.mp4"
            first.write_bytes(b"v")
            second.write_bytes(b"v")

            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path):
                IndexIngestTests._reset_index_module()
                self.assertTrue(index.register_video(
                    str(first), "First Channel", "Shared Title",
                    video_id="first000001"))
                self.assertTrue(index.register_video(
                    str(second), "Second Channel", "Shared Title",
                    video_id="second00001"))
                self.assertTrue(index.mark_video_no_speech(str(second)))

                self.assertNotEqual(
                    index.video_tx_status(
                        title="Shared Title", channel="First Channel"),
                    "no_speech",
                )
                self.assertEqual(
                    index.video_tx_status(
                        title="Shared Title", channel="Second Channel"),
                    "no_speech",
                )
                IndexIngestTests._reset_index_module()

    def test_archive_stats_exclude_hidden_duplicate_and_partial_rows(self) -> None:
        from backend import archive_scan
        ...
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
            ...
            with mock.patch.object(index, "TRANSCRIPTION_DB", db_path), \
                ...
                conn.executemany(
                    "INSERT INTO videos("
                    "filepath, title, channel, video_id, tx_status, "
                    "is_duplicate_of, availability"
                    ") VALUES (?, ?, ?, ?, ?, ?, ?)",
                    [
...
class DriftScanTests(unittest.TestCase):
    ...
    def test_apply_channel_reconstruction_uses_recovered_upload_date(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            # v80: reconstruction carries the jsonl's video id into the
            # header's trailing (youtu.be/<id>) field.
            self.assertIn("===(Video), (02.03.2024), (0:00), "
                          "(RECOVERED-FROM-JSONL), (youtu.be/abc123_def4)===",
                          content)
