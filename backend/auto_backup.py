...
program's PC. This module maintains `<archive root>\\YTArchiver Info\\`:
  ...
  ytarchiver_backup_<ts>.zip    — the same full app-state export the
                                  ...
                                  schedule. The configured newest stay;
...
KEEP_BACKUPS = AUTO_BACKUP_KEEP_DEFAULT
...
def _about_text() -> str:
    ...
    keep_backups = normalize_auto_backup_keep(load_config().get("auto_backup_keep"))
...
-------------------------------
  ...
  {BACKUP_PREFIX}*.zip — snapshots of the app's state:
                             ...
                             {keep_backups} scheduled backups are kept, as
                             selected in Health > Backups. Older scheduled
                             backups are removed after a new backup succeeds.
...
def _rotate_backups(info_dir: str, output_dir: str) -> int:
    """Keep the configured number of newest scheduled backup ZIPs.
